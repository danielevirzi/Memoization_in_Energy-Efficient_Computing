...
class RunnerConfig:
    ...
    input_size_options = [
        (tuple(map(tuple, np.random.rand(55, 55))), tuple(map(tuple, np.random.rand(5, 5)))),
        (tuple(map(tuple, np.random.rand(44, 44))), tuple(map(tuple, np.random.rand(4, 4)))),
        (tuple(map(tuple, np.random.rand(33, 33))), tuple(map(tuple, np.random.rand(3, 3))))
    ]
    input_description = [
        "55x55 matrix, 5x5 kernel",
        "44x44 matrix, 4x4 kernel",
        "33x33 matrix, 3x3 kernel"
    ]
